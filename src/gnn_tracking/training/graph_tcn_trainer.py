...
import numpy as np
import pandas as pd
from utils.losses import EdgeWeightLoss, PotentialLoss, BackgroundLoss, ObjectLoss
from utils.training import binary_classification_stats

class GraphTCNTrainer:
    def __init__(self, model, loaders, device='cpu',
                 lr=5*10**-4, q_min=0.01, sb = 1, epochs=1000,
                 object_loss_mode='purity', predict_track_params=False):
        ...
        self.object_loss = ObjectLoss(q_min=q_min, device=device,
                                      sb=sb, mode=object_loss_mode)
    ...
    def train_step(self, epoch):
        ...
        for batch_idx, data in enumerate(self.train_loader):
            ...
            loss_W = self.edge_weight_loss(self.W, self.B, self.H,
                                           self.Y, self.L)
            loss_V = self.potential_loss(self.W, 
                                         self.B,
                                         self.H,
                                         self.Y, 
                                         self.L)
            loss_B = self.background_loss(self.W, 
                                          self.B,
                                          self.H,
                                          self.Y, 
                                          self.L)
