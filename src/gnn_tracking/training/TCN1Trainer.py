...
from models.condensation_loss import PotentialLoss, BackgroundLoss, EdgeWeightLoss, ObjectLoss
...
class TCNTrainer:
    def __init__(self, args, model, loaders, device='cpu',
                 constrain_loss=False):
        self.args = args
        ...
        self.background_loss = BackgroundLoss(q_min=args.q_min, device=device,
                                              sb=args.sb)
        self.object_loss = ObjectLoss(q_min=args.q_min, device=device,
                                      sb=args.sb, mode='purity')
        
        # quantities to predict 
        self.W = torch.empty(1, dtype=torch.float, # edge weights 
                             device=device)
        self.B = torch.empty(1, dtype=torch.float, # condensation likelihoods
                             device=device)
        self.H = torch.empty(1, dtype=torch.float, # clustering coordinates
                             device=device)
        self.Y = torch.empty(1, dtype=torch.float, # edge truth labels
                             device=device)
        self.L = torch.empty(1, dtype=torch.float, # hit truth labels
                             device=device)
        self.P = torch.empty(1, dtype=torch.float, # track parameters
                             device=device)
    ...
    def build_constrained_optimizer(self):
        args = self.args
        loss_W = lambda: self.edge_weight_loss(self.W, self.B, self.H, 
                                               self.Y, self.L)
        loss_B = lambda: self.background_loss(self.W, self.B, self.H, 
                                              self.Y, self.L)
        ...
        optimizer = mdmm_module.make_optimizer([self.W, self.B, self.H, 
                                                self.Y, self.L],
                                               lr=args.learning_rate,
    ...
    def validate(self):
        ...
        for batch_idx, (data, f) in enumerate(self.val_loader):
            ...
            if args.predict_track_params:
                self.W, self.H, self.B, self.P = self.model(data.x, data.edge_index, 
                                                            data.edge_attr)
            else:
                self.W, self.H, self.B = self.model(data.x, data.edge_index, 
                                                    data.edge_attr)
            self.Y, self.W = data.y, self.W.squeeze(1)
            self.L = data.particle_id
            loss = self.edge_weight_loss(self.W, self.B, self.H,
                                         self.Y, self.L)
    ...
    def test(self, epoch, thld=0.5):
        ...
        with torch.no_grad():
            for batch_idx, (data,f) in enumerate(self.test_loader):
                ...
                loss_W = self.edge_weight_loss(self.W, self.B, self.H,
                                               self.Y, self.L).item()
                loss_V = self.potential_loss(self.W, self.B, self.H,
                                             self.Y, self.L).item()
                loss_B = self.background_loss(self.W, self.B, self.H,
                                              self.Y, self.L).item()
                loss_P = self.object_loss(self.W, self.B, self.H, self.P,
                                          self.Y, self.L, self.T, self.R).item()
                acc, TPR, TNR = binary_classification_stats(self.W, self.Y, thld)
    ...
    def train(self, epoch):
        ...
        for batch_idx, (data, f) in enumerate(self.train_loader):
            data = data.to(self.device)
            if args.predict_track_params:
                self.W, self.H, self.B, self.P = self.model(data.x, 
                                                            data.edge_index, 
                                                            data.edge_attr)
            ...
            loss_B = self.background_loss(self.W, self.B, self.H,
                                          self.Y, self.L)
            loss_P = self.object_loss(self.W, self.B, self.H, self.P,
                                      self.Y, self.L, self.T, self.R)
            loss = self.multi_loss([loss_W, loss_V, loss_B, loss_P])
            self.optimizer.zero_grad()
            loss.backward()
            self.optimizer.step()
            if not (batch_idx%10): 
                logging.info(f'Epoch {epoch} ({batch_idx}/{len(self.train_loader)}):'
...
def main(args): 
    ...
    loaders = get_dataloaders(args.indir, args.n_train,
                              args.n_test, args.n_val, shuffle=False,
                              params = params)
    ...
    model = TCN(5, 4, 5, 
                predict_track_params=predict_track_params).to(device)
