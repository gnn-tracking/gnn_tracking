...
def get_graph_evtids(graphs):
    return np.array([int(g.split('0000')[-1].split('_s')[0])
                     for g in graphs])

def sort_graph_paths(paths):
    evtids = np.array([int(g.split('0000')[-1].split('_s')[0])
                       for g in paths])
    unique_evtids = np.sort(np.unique(evtids))
    sorted_paths = []
    for evtid in unique_evtids:
        evtid_paths = paths[evtids==evtid]
        sectors = np.array([int(g.split('_s')[-1].split('.')[0])
                            for g in evtid_paths])
