...
def empty_graph():
    singles = {k: np.array([]) 
               for k in ['x', 'hit_id', 'particle_id', 'y']}
    doubles = {k: np.array([[], []])
               for k in ['edge_index', 'edge_hit_id']}
...
def select_hits(hits, truth, particles, pt_min=0, endcaps=False,
    ...
    if (endcaps):
        vlids.extend([(7,14), (7,12), (7,10), (7,8),
                      (7,6), (7,4), (7,2),
                      (9,2), (9,4), (9,6),
                      (9,8), (9,10), (9,12), (9,14)])
    ...
    hits = (hits[['hit_id', 'r', 'phi', 'eta', 
                  'x', 'y', 'z', 'u', 'v', 'layer', 'module_id']]
            .merge(truth[['hit_id', 'particle_id', 'pt', 'eta_pt', 'q', 'vx', 'vy']], 
                   on='hit_id'))
    ...
    if relabel_pids: hits, particles = relabel_pids(hits, particles)
    return hits, particles

def select_edges(hits1, hits2, layer1, layer2,
                 phi_slope_max, z0_max, dR_max=0.5,
                 uv_approach_max=100,
                 module_map=[], use_module_map=False):
    ...
    selected_edges = pd.DataFrame({'index_1': hit_pairs['index_1'][good_edge_mask],
...
def correct_truth_labels(hits, edges, y, particle_ids):
    ...
    barrel_to_endcaps = {(0,4), (1,4), (2,4), (3, 4),    # barrel to l-EC
                         (0,11), (1,11), (2,11), (3, 11)} # barrel to r-EC
    precedence = {(0,4): 0, (1,4): 1, (2,4): 2, (3,4): 3,
