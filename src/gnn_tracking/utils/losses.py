...
class PotentialLoss(torch.nn.Module):
    ...
    def V_repulsive(self, x, x_alpha, q_alpha):
        ...
        hinges = torch.maximum(torch.zeros(len(x)).to(self.device),
                               diffs)
...
class BackgroundLoss(torch.nn.Module):
    ...
    def forward(self, w, beta, x, y, particle_id):
        return self.background_loss(beta, x, particle_id,
                                    self.q_min, self.sb)
...
class ObjectLoss(torch.nn.Module):
    ...
    def MSE(self, p, t):
        return torch.sum(mse_loss(p, t, reduction='none'),
                         dim=1)
    ...
    def forward(self, W, beta, H, pred, Y, particle_id, track_params,
                reconstructable):
        mask = reconstructable > 0
        return self.object_loss(pred[mask], beta[mask], 
                                track_params[mask], particle_id[mask])
