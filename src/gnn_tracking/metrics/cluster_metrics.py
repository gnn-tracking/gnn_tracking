...
class TrackingMetrics(TypedDict):
    ...
    #: of particles
    perfect: float
    #: Fraction of particles with at least one valid cluster satisfying the perfect
    #: match homogeneity threshold
    perfectly_homogeneous: float
    #: Fraction of particles with a valid cluster containing all of their hits
    perfectly_inclusive: float
...
_tracking_metrics_nan_results: TrackingMetrics = {
    ...
    "perfect": float("nan"),
    "perfectly_homogeneous": float("nan"),
    "perfectly_inclusive": float("nan"),
    "lhc": float("nan"),
...
def tracking_metric_df(h_df: pd.DataFrame, predicted_count_thld=3) -> pd.DataFrame:
    ...
    c_df["perfectly_homogeneous"] = (c_df["maj_frac"] > 0.99) & c_df["valid_cluster"]
    c_df["perfectly_inclusive"] = (c_df["maj_pid_hits"] == c_df["maj_hits"]) & c_df[
        "valid_cluster"
    ]
    c_df["perfect_match"] = c_df["perfectly_homogeneous"] & c_df["perfectly_inclusive"]
...
def count_tracking_metrics(
    ...
    n_perfectly_homogeneous = c_df.loc[
        c_mask & c_df["perfectly_homogeneous"], "maj_pid"
    ].nunique()
    n_perfectly_inclusive = c_df.loc[
        c_mask & c_df["perfectly_inclusive"], "maj_pid"
    ].nunique()
    ...
    r: TrackingMetrics = {
        ...
        "perfectly_homogeneous": zero_division_gives_nan(
            n_perfectly_homogeneous, n_particles
        ),
        "perfectly_inclusive": zero_division_gives_nan(
            n_perfectly_inclusive, n_particles
        ),
